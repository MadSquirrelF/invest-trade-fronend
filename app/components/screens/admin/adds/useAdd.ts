...
export const useAdd = () => {
  ...
  const queryData = useQuery([`add list`, debouncedSearch], () => AddService.getAll(debouncedSearch), {
  ...
  const { mutateAsync: createAsync } = useMutation(`create add`, () => AddService.createAdd(), {
    onError: (error) => {
  ...
  const { mutateAsync: deleteAsync } = useMutation(`delete add`, (AddId: string) => AddService.deleteAdd(AddId), {
    onError: (error) => {
