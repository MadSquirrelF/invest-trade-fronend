...
export const useWork = () => {
  ...
  const queryData = useQuery([`work list`, debouncedSearch], () => WorkService.getAll(debouncedSearch), {
  ...
  const { mutateAsync: createAsync } = useMutation(`create work`, () => WorkService.createWork(), {
    onError: (error) => {
  ...
  const { mutateAsync: deleteAsync } = useMutation(`delete work`, (WorkId: string) => WorkService.deleteWork(WorkId), {
    onError: (error) => {
